...
for lr in learning_rates:
    for batch_size in batch_sizes:
        for opt_name, opt_func in optimizers_dict.items():
            ...
            model = ResNetModel(num_classes=3).to(device)  # Ensure model is initialized for 3 classes
            optimizer = optimizers_dict[opt_name](model.parameters(), lr=lr)
            criterion = nn.CrossEntropyLoss()  # Updated to CrossEntropyLoss for multi-class
...
#test dataları
test_directory = "C:/Users/tayla/Downloads/test_png"
...
if x==0:
    ...
    #grad_cam uygulama
    def apply_grad_cam(model, images, target_layer):
        gradients = []
        activations = []
    ...
    # grad camin bir imageı process etmesi ve visualize etmesi icin gereken fonksiyon
    def process_and_visualize_grad_cam(image_path, model, val_transform, target_layer):
        ...
        plt.subplot(1, 2, 1)
        plt.imshow(image_np)
        plt.title(f'{image} predicted as: {predicted_quality}')
        plt.axis('off')
    ...
    # test imageları
    test_directory = "C:/Users/tayla/Downloads/test_png"
    png_files = [file for file in os.listdir(test_directory) if file.endswith('.png')]
